...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (anim)
        {
            ...
            {
                if (p.type == AnimatorControllerParameterType.Trigger && p.nameHash == hashHit)
                    hasHit = true;
                if (p.type == AnimatorControllerParameterType.Bool && p.nameHash == hashIsDead)
                    hasIsDead = true;
            }
        }
    }
